...
Do the same but only with the smaller, more reliable melody sets:
...
And again with the embeddings approach


```{r}

dat_we2 <- readRDS('../../data/all_sim_m2m.rds')
# There is also m2a
  
dat_we2 <- dat_we2$as_wide()
...
dat_we2 <- dat_we2 %>% 
  rowwise() %>% 
  mutate(melody = glue::glue("notes: { {pitch} }, durations: {duration}"),
         melody = paste0("{ ", melody, " }")) %>% 
  ungroup() %>% 
  rowwise() %>% 
  mutate(embedding = list( get_chatgpt_word_embedding(melody) )) %>% 
...
dat_we2 <- dat_we2 %>% 
  mutate(melody_no = readr::parse_number(name)) %>% 
  select(rating_pair, melody_no, embedding, sim_ratings) %>% 
  pivot_wider(everything(), names_from = "melody_no", values_from = "embedding") %>% 
  rename(melody_1 = `1`,
         melody_2 = `2`) %>%  
  rowwise() %>% 
...
# All very high cosine sims...

save(dat_we2, file = 'output/dat_we2.rda')
...
chatgpt_word_embedding_lm2 <- lm(sim_ratings ~ cosine_sim, data = dat_we2)
...
summary(chatgpt_word_embedding_lm2)
